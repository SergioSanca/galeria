...
app.config["MAIL_USERNAME"] = "SergioSanchez"
app.config["MAIL_PASSWORD"] = "17940-01"
...
def galeria():
    ...
    if t == None:
        ...
        li = misfondos.find({"tags": {"$in": [t]}})
        estilos[t] = "active"
    return render_template("index.html", activo=estilos, lista=li)
...
def insertar():
    ...
    if f.filename == "":
        ...
        if archivo_permitido(f.filename):
            ...
            return render_template("aportar.html", mensaje="¡El archivo \
                                   indicado no es una imagen!")
    ...
    misfondos.insert({"titulo": tit, "descripcion": desc, "fondo": archivo,
                      "tags": tags})
    return redirect("/")
...
def enviaemail():
    ...
    msg.html = render_template("email.html", titulo=documento["titulo"],
                               descripcion=documento["descripcion"])
